...
class DeepSeekConnectorNode(Node):
    def __init__(self):
        ...
        self.model = self.get_parameter('model').value or 'deepseek-v4-flash'
    ...
    def _process_queue(self):
        while rclpy.ok():
            ...
            if not self.api_key:
                self._publish_command_status(command_id, 'failed', 'No DEEPSEEK_API_KEY configured')
            ...
            try:
                ...
                self.get_logger().info(f'[{command_id[:8]}] DeepSeek response published')
            except Exception as e:
                reason = f'DeepSeek API error: {e}'
    ...
    def _build_prompt(self, command, fleet_state):
        ...
        return (
            ...
            '      "waypoints": [{"x": <float>, "y": <float>}, {"x": <float>, "y": <float>}],\n'
            '      "priority": <int 1-5>\n'
            '    },\n'
            '    {\n'
            '      "action": "navigate",\n'
            ...
            '- action must be "navigate", "set_state", "rotate", "emergency_stop", "clear_emergency_stop", "go_home", or "patrol"\n'
            ...
            '- navigate: provide one of: "waypoints" [{x,y} or {x,y,yaw} or {lat,lon}], "waypoint_name", "waypoint_names" [names], or "relative" {"forward": meters, "right": meters}. yaw is optional (radians, robot faces that direction at goal). priority 1-5 (optional, default 3).\n'
            '- For sequential commands like "go to A then B" or "go to x=1,y=1 and then x=2,y=2", generate multiple entries in the "waypoints" array. The bridge drives to each in order.\n'
            '- "waypoint_names": use for multiple named locations like "point_a then charging_dock". The bridge resolves each name and drives to them in order.\n'
            ...
            '- patrol: patrols multiple waypoints with contingencies. Use for "patrol A, B, C", "check perimeter", "inspect locations". Contingencies: "on_obstacle" (stop|return_home|skip_and_continue), "on_stuck" (abort|rotate_and_continue). If user says "if obstacle, return home" use on_obstacle: "return_home".\n'
    ...
    def _call_deepseek(self, prompt):
        ...
        try:
            ...
            raise RuntimeError(f'{e.code} {e.reason}: {error_body}')
